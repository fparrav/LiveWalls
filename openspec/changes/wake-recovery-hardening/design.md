...
Display-scoped rebuild: the fresh rebuild takes an optional set of target displays (`CGDirectDisplayID`). A wake, a manual change, or a global health failure passes *no* target set and rebuilds every window as above. A stall confined to a subset of displays — the per-window render-advance probe (D1) reports which — passes exactly those display IDs, and the rebuild then tears down and recreates only the matching windows, leaving the healthy displays' `AVQueuePlayer` / `AVPlayerLooper` / `AVPlayerLayer` instances untouched and their playback uninterrupted; the first-frame probe is likewise scoped to the rebuilt windows. The display-scoped path deliberately skips `BookmarkActor.reconcile()` (D6) — draining security-scoped access to zero would revoke the still-live windows' access — and relies on per-URL ref-counting instead: the torn-down windows release their refs and the recreated ones re-acquire, so the shared bookmark URL never drops to zero while a window still needs it. It therefore requires `bookmarkRefCount` (D6 / task 2.6) to be enabled; with that kill-switch off, recovery falls back to the global rebuild. `didWake` stays global because a suspend/wake suspends every display's compositor.

Rationale: rebuilding all displays because one stalled needlessly interrupts healthy playback (a visible flicker on every monitor) and widens the blast radius of a recovery bug; scoping the rebuild to the stalled display keeps recovery proportional to the failure.
